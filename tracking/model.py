...
class AB3DMOT(object):			  	
	def __init__(self, calib=None, oxts=None, img_dir=None, vis_dir=None, hw=None, log=None, ID_init=0):                    
		...
		self.calib = calib
		self.oxts = oxts
		self.affi_process = True # todo: check what this is 

		#
		# Finetune Parameters for Dataset
		#
		algm, metric, thres, min_hits, max_age = 'hungar', 'dist3d_bottom', 2, 3, 4
		# add negative due to it is the cost
		if metric in ['dist_3d', 'dist3d_bottom', 'dist_2d', 'm_dis', 'dist_3dspace']: 
			thres *= -1	
		...
		if self.metric in ['dist_3d', 'dist_2d', 'm_dis', 'dist3d_bottom']: self.max_sim, self.min_sim = 0.0, -100.
	...
	def track(self, dets_all):
		...
		# ego motion compensation was removed because not all teams provide that
